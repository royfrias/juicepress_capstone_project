...
export default function Home() {
  ...
  async function fetchData() {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_SERVER_URL}/announcements`,
        {
          headers: {
  ...
  async function createAnnouncement() {
    ...
    const response = await fetch(
      `${import.meta.env.VITE_SERVER_URL}/createannouncement`,
      {
        method: "POST",
        headers: {
          authorization: localStorage.getItem("jwt-token"),
  ...
  const updateAnnouncement = async () => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_SERVER_URL}/announcement/${
  ...
  const handleDeleteClick = async (id) => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_SERVER_URL}/announcement/${id}`,
        {
          method: "DELETE",
          headers: {
            authorization: `Bearer ${localStorage.getItem("jwt-token")}`,
  ...
  async function getUsername() {
    ...
    const response = await fetch(
      `${import.meta.env.VITE_SERVER_URL}/user/username`,
      {
        method: "GET",
        headers: {
          authorization: localStorage.getItem("jwt-token"),
  ...
  async function getAdminUsername() {
    ...
    //using fetch to obtain user last name and first name from database
    const response = await fetch(
      `${import.meta.env.VITE_SERVER_URL}/admin/adminUsername`,
      {
        method: "GET",
        headers: {
              ...
              <div className="card lg:card-side bg-base-100 shadow-xl">
                ...
                <div className="card-body">
                  ...
                  {announcement.timestamp && (
                    <p>
                      {formatDistanceToNow(parseISO(announcement.timestamp))}{" "}
                      ago
                    </p>
                  )}
          ...
          >
            ...
            <div className="flex flex-col space-y-9">
              <button
                ...
                className="block btn rounded-full bg-blue-magic hover:bg-clean-green outline outline-offset-1 outline-black w-36 mx-14"
              >
                Profile
              </button>
              <button
                onClick={() => navigateSlack()}
                className="block btn rounded-full bg-blue-magic hover:bg-clean-green outline outline-offset-1 outline-black w-36 mx-14"
              >
                Slack
              </button>
              ...
              {token ? (
                <>
                  <button
                    className="block btn bg-blue-magic rounded-full hover:bg-clean-green outline outline-offset-1 outline-black w-42 mx-14"
                  ...
                  <dialog className="modal-box" id="my_modal_2">
                    <form onSubmit={handleAnnouncementSubmit}>
                      ...
                      <input
                        ...
                        accept="video/*"
                        onChange={(e) =>
                          setAnnouncementVideo(e.target.files[0])
                        }
                      />
                      <button
                        className="btn bg-blue-magic rounded-full px-7 py-3 hover:bg-mango-madness font-semibold mt-3 ml-33"
                        type="submit"
                      >
                        Submit
                      </button>
                    ...
                    <button
                      className="btn bg-clean-green rounded-full px-6 py-3 hover:bg-mango-madness font-semibold mt-3 ml-33"
                      onClick={() =>
                        document.getElementById("my_modal_2").close()
                      }
                    >
